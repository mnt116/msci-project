...
saveburnin = True # save plot of step-by-step progress of mcmc as 21cm_burnin.png

# CORNER PLOT
plotcorner = True # plot corner plot of mcmc results
savecorner = True # save corner plot as 21cm_cornerplot.png
 
# MODELS PLOT
plotmodels = True # plot models vs simulated data for 100 random points in chain
savemodels = True # save plot of models vs simulated data as 21cm_modelsplot.png
...
simsig = simfore + np.random.normal(0, 0.001, len(freqfull)) # foreground + absorption dip + thermal noise
...
pos = np.array([a0,a1,a2,a3,a4]) + 1e-4*np.random.randn(32,5) # initial values of model parameters
nwalkers, ndim = pos.shape # number of walkers, and dimensions of sampler

n_steps = 20000 # number of steps for mcmc
...
if plotburnin == True:
    fig, axes = plt.subplots(5, figsize=(10, 5), sharex=True) # plotting step-by-step progress of mcmc
...
flat_samples = sampler.get_chain(discard=12000, thin=15, flat=True) # flattened chain; discard burn-in values and thin chain
...
if plotmodels == True: # plot models vs simulated data for 100 random points in chain
    ...
    for i in s_inds:
        sp = flat_samples[i]
        model = sp[0] + sp[1]*ff + sp[2]*ff**2 + sp[3]*ff**3 + sp[4]*ff**4
